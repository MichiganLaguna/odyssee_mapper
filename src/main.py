...
def add_path(node_a, node_b, length):
    ...
    for path in list_all_paths:
        if (
            re.search(f"({node_a})", path[0]) is not None
            and re.search(f"({node_b})", path[1]) is not None
        ) or (
            re.search(f"({node_b})", path[0]) is not None
            and re.search(f"({node_a})", path[1]) is not None
        ):
            return -1
    ...
    def add_node_verif(node_name):
        if add_node(node_name) is not None:
            messagebox.showerror(
                title="Node Error", message="I'm afraid this node already exist !"
            )
        entry_update(add_node_entry)

    add_node_entry = Entry(add_node_frame)
    add_node_button = Button(
        add_node_frame, text="Add", command=lambda: add_node_verif(add_node_entry.get())
    )
...
def open_remove_node_window():
    ...
    remove_node_button = Button(
        remove_node_frame, text="Remove", command=rem_node_verif
    )
...
def open_add_path_window():
    ...
    add_path_combobox_start = AutocompleteCombobox(add_path_frame, width="50")
    add_path_label_end = Label(add_path_frame, text="End")
    add_path_combobox_end = AutocompleteCombobox(add_path_frame, width="50")
    ...
    def add_path_verif():
        ...
        if node_start == node_end:
            messagebox.showerror(
                title="Path Error",
                message="I'm afraid you can't make a path between the same point !",
            )
        if node_start is None or node_end is None:
            messagebox.showerror(
                title="Path Error", message="I'm afraid you need to select more nodes !"
            )
        if add_path(node_start, node_end, length) is not None:
            messagebox.showerror(
                title="Path Error", message="I'm afraid this path already exist !"
            )
        list_nodes_update(add_path_combobox_start)
...
def open_remove_path_window():
    ...
    list_paths_update(remove_path_combobox)
    remove_path_button = Button(
        remove_path_frame, text="Remove", command=verif_remove_path
    )
...
# %%
nodes_menu = Menu(main_menu, tearoff=0)
nodes_menu.add_command(label="Add a node...", command=open_add_node_window)
nodes_menu.add_command(label="Remove a node...", command=open_remove_node_window)

paths_menu = Menu(main_menu, tearoff=0)
paths_menu.add_command(label="Add a path...", command=open_add_path_window)
paths_menu.add_command(label="Remove a path...", command=open_remove_path_window)
question_mark_menu = Menu(main_menu, tearoff=0)
question_mark_menu.add_command(label="Help !")
question_mark_menu.add_command(label="About us...")

# %%
main_menu.add_cascade(label="Nodes", menu=nodes_menu)
main_menu.add_cascade(label="Paths", menu=paths_menu)
main_menu.add_cascade(label="?", menu=question_mark_menu)
